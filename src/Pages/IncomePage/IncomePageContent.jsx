...
const IncomePageContent = () => {
  ...
  const fetchCategories = async () => {
    try {
      const url = `http://localhost:8080/category/getСostCategories/доходы-${personId}`;
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/income/${filters.month}-${filters.year}-${personId}`
      );
      const data = await response.json();
      if (Array.isArray(data)) {
        setData(data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/income/addIncome", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`Ошибка: ${errorData || "Не удалось добавить доход"}`);
      }
    } catch (error) {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.content}>
        <h1 className={styles.contentTitle}>Доходы</h1>
